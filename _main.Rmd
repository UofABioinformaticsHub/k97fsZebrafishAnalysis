...
<!--chapter:end:01-DEGene-Analysis.Rmd-->
...
<!--chapter:end:02-DAProtein-Analysis.Rmd-->
...
<!--chapter:end:03-Gene-Set-Testing-with-FRY.Rmd-->
...
<!--chapter:end:04-Enrichment-Analysis.Rmd-->
...
# This object is taken from the DE gene analysis done using limma and edgeR.
...
Import the `DGEList` object previous made in the DE gene analysis, `geneCounts`. This object contains three data.frames, `sample` (sample metadata), `counts` (gene counts in cpm), and `genes` (gene annotation from *Biomart*). The data has already been normalised (TMM method), which can be seen in Figure \@ref(fig:WGCNA-Boxplots-Normalised-Gene-Expression).
...
According to *WGCNA*'s [FAQ page](https://labs.genetics.ucla.edu/horvath/CoexpressionNetwork/Rpackages/WGCNA/faq.html), genes can be filtered by mean expression of variance since low-expressed or non-varying genes usually represent noise, which is undesirable when forming a co-expression network. Below, we will filter by both mean expression and variance to retain genes which tend to be more highly expressed and variable across samples. The effects of filtering via expression (>1.5cpm in at least 6 RNA-seq libraries) and variance (standard deviation above the 25th quantile) is shown in Figure \@ref(fig:WGCNA-Densityplot-Effect-of-Filtering-Gene-Expression)
...
humanExprsUniqueGenes <- humanExprs %>% 
  ...
  as.matrix %>% 
  collapseRows(rowGroup = humanExprs$gene_assignment, rowID = rownames(.), method = "MaxMean")
...
humanToZebrafish <- getBM(
  attributes = c("ensembl_gene_id", "drerio_homolog_ensembl_gene", 
                 "drerio_homolog_orthology_type", "drerio_homolog_perc_id", 
                 "drerio_homolog_goc_score", "drerio_homolog_orthology_confidence"),
  values = list("ensembl_gene_id" = geneIDs$ensembl_gene_id),
  filters = c("ensembl_gene_id"), mart = huMart)
...
finalHumanGeneList <- humanToZebrafish %>% 
  filter(drerio_homolog_ensembl_gene != "", drerio_homolog_ensembl_gene %in% colnames(zebrafishWGCNAGenes)) %>%
...
finalHumanGeneList %>% 
  ...
  ggplot(aes(x = sample, y = geneexp, fill = as.factor(psen1_mutation))) + 
  geom_boxplot(outlier.colour = "#888888", outlier.size = 0.25, 
               notch = TRUE, outlier.fill=NULL, outlier.shape=19) +
...
writexl::write_xlsx(exportFunctionalEnrichmentResults, file.path(wgcnaDir, "FunctionalEnrichment_anRichment.xlsx"))
...
traits <- readRDS(file.path(dataDir, "de_genes", "designMatrix.rds")) 
...
```{r Module-Heatmap-Human, echo=FALSE, fig.height=6, fig.cap="Module expression changes (and p-values) between human samples."}
...
```{r Heatmap-Overlap, fig.height=11, fig.cap="Heatmap showing overlap between zebrafish modules (rows) and human modules (columns). "}
...
```{r Sankey-Diagram, fig.height=11}
...
sankeyNetwork(Links = module_overlap, 
              ...
              iterations = 500,
              nodePadding = 10,
              colourScale = JS("d3.scaleOrdinal().range(['black', '#2196F3', '#8D6E63', '#18FFFF', '#00C853', '#B71C1C', '#00838F', '#8BC34A', 'greenyellow', '#9E9E9E', '#455A64', '#84FFFF', '#FFEB3B', '#FF4081', 'midnightblue', '#F48FB1', '#7E57C2', '#F44336', 'royalblue', 'salmon', 'tan', 'turquoise', '#FDD835']).domain(['black', 'blue', 'brown', 'cyan', 'darkgreen', 'darkred', 'darkturquoise', 'green', 'greenyellow', 'grey', 'grey60', 'lightcyan', 'lightyellow', 'magenta', 'midnightblue', 'pink', 'purple', 'red', 'royalblue', 'salmon', 'tan', 'turquoise', 'yellow'])"))

```


<!--chapter:end:05-WGCNA.Rmd-->
